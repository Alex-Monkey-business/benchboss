...
ok('hele klubbens lagliste er tilgjengelig som utvei', await p.getByRole('button',{name:/Vis alle \d+/}).count()===1)
...
// Alle lagene i årskullet hentes. Det treneren velger er hvilke av dem de er
// lagleder for: ett lag er valgt fra start, flere må velges.
const lagKort=await p.locator('.kig__lag').count()
const valgte=await p.locator('.kig__lag--valgt').count()
ok('spørsmålet er hvilke lag du leder', /Hvilke lag er du ansvarlig for/.test(await p.locator('.kig__tittel').innerText()))
ok('ett lag er valgt fra start, flere er ikke', lagKort>0 && valgte===(lagKort===1?1:0), `${valgte}/${lagKort}`)
if (!valgte) await p.locator('.kig__lag').first().click()
const hentKnapp=p.getByRole('button',{name:/Hent lagene og kampene/})
...
ok('alle lagene i årskullet er opprettet', Number(sql(`select count(*) from teams where cohort_id='${kullId}'`))===lagKort, `${sql(`select count(*) from teams where cohort_id='${kullId}'`)} av ${lagKort}`)
ok('treneren er bare lagleder for det valgte laget', Number(sql(`select count(distinct team_id) from team_coaches where cohort_id='${kullId}'`))<=1, sql(`select count(distinct team_id) from team_coaches where cohort_id='${kullId}'`))
...
await p.waitForTimeout(700)
if (!(await p.locator('.kig__lag--valgt').count())) await p.locator('.kig__lag').first().click().catch(()=>{})
await p.getByRole('button',{name:/Hent lagene og kampene/}).click().catch(()=>{})
await p.waitForTimeout(9000)
