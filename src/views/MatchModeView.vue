...
function flushLineupSave() {
  ...
  lineupDirty = false
  saveSetup(matchId, setupPatch()).catch(() => { /* best effort */ })
}
...
function scheduleSetupSave() {
  ...
  lineupTimer = setTimeout(() => {
    lineupDirty = false
    saveSetup(matchId, setupPatch()).then(markSaved).catch(reportError)
  }, 600)
}
...
// ── Bytt plass med dra (long-press + dra) — både oppsett og live ──────────────
...
// Skal ghost-drakta vises som keeper?
const dragIsKeeper = computed(() => {
  if (!dragId.value) return false
  return phase.value === 'setup' ? dragFromSlot.value === 'gk' : roleOf(dragId.value) === 'keeper'
})

function pitchXY(clientX, clientY) {
  const r = pitchEl.value?.getBoundingClientRect()
...
const atHalfBreak = computed(() =>
  ...
  session.value?.clock_base_seconds === period.value * halfSeconds.value &&
  period.value < periodCount.value
)
const phaseLabel = computed(() => {
  if (atHalfBreak.value) return periodCount.value === 2 ? 'Halvtid' : 'Pause'
  if (session.value?.status === 'paused') {
    return (session.value?.clock_base_seconds || 0) >= periodCount.value * halfSeconds.value ? 'Full tid' : 'Pauset'
  }
...
// Auto-pause når klokka når omgangsslutt. Guard mot dobbel-trigging.
...
<template>
  <div class="mm">
    ...
    <div v-else-if="phase === 'live'" class="mm__live">
      ...
      <div ref="pitchEl" class="pitch pitch--live">
        <div class="pitch__turf" aria-hidden="true"><span></span><span></span><span></span><span></span><span></span><span></span></div>
        <div class="pitch__lines" aria-hidden="true">
          <span class="pitch__circle"></span>
          <span class="pitch__halfway"></span>
